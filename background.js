...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    if (message.type === 'checkContent') {
        chrome.storage.sync.get(["blockedKeywords", "adaptiveFilteringEnabled"], (settings) => {
            ...
            const adaptiveEnabled = settings.adaptiveFilteringEnabled;
...
function trackUserBehavior(url) {
    ...
    // Check if the URL matches known risky patterns
...
// Function to analyze content using Prompt API
...
async function rewriteContent(content, sendResponse) {
    ...
    try {
        ...
        const data = await response.json();
        sendResponse({ rewritten: data.rewritten_text });
    } catch (error) {
